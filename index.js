...
app.post("/api/admin/payments", verifyToken, verifyAdmin, async (req, res) => {
  try {
...
app.get("/api/vendor/tickets", verifyToken, verifyVendor, async (req, res) => {
  try {
    const tickets = await ticketsCollection
...
app.get("/api/vendor/revenue", verifyToken, verifyVendor, async (req, res) => {
  try {
    const vendorTickets = await ticketsCollection
      .find({ vendorEmail: req.user.email }, { projection: { _id: 1 } })
      .toArray();
...
app.post("/api/confirm-booking", async (req, res) => {
  ...
  try {
    ...
    if (session.payment_status === "paid") {
      ...
      if (exists) return res.send({ success: true, message: "Already saved" });
